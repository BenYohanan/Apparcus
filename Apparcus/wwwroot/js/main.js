...
function getProjectById(id) {
    $.ajax({
        ...
        success: function (project) {
            ...
            $('#qrCodeImage').attr('src', 'data:image/png;base64,' + project.qrCodeBase64);
            var encodedLink = encodeURIComponent(project.supportLink);
            var encodedText = encodeURIComponent(`Support ${project.name} - Target: ₦${project.amountNeeded}`);
            $('#shareWhatsapp').attr('href', `https://wa.me/?text=${encodedText}%20${encodedLink}`);
            $('#shareTwitter').attr('href', `https://twitter.com/intent/tweet?text=${encodedText}&url=${encodedLink}`);
            $('#shareFacebook').attr('href', `https://www.facebook.com/sharer/sharer.php?u=${encodedLink}`);
            $('#shareTelegram').attr('href', `https://t.me/share/url?url=${encodedLink}&text=${encodedText}`);
        },
        error: function () {
...
document.getElementById('openLinkBtn')?.addEventListener('click', () => {
    window.open(document.getElementById('shareLinkInput').value, '_blank');
});

// Copy QR Code as Image to Clipboard (Clean version)
document.getElementById('copyQrImageBtn')?.addEventListener('click', async () => {
    const qrImage = document.getElementById('qrCodeImage');
    const button = document.getElementById('copyQrImageBtn');

    if (!qrImage.src || !qrImage.src.startsWith('data:image')) {
        errorAlert("QR code is still loading...");
        return;
    }
    ...
    try {
        ...
        setTimeout(() => {
            button.innerHTML = '<i class="ti ti-qrcode"></i>';
